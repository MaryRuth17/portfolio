...
// Simple SplitText replacement since we don't have GSAP premium plugins
...
const Shuffle: React.FC<ShuffleProps> = ({
  ...
  const splitRef = useRef<{ chars: HTMLElement[] } | null>(null);
  ...
  useEffect(() => {
    ...
    // Set ready state immediately so text is visible
    setReady(true);
  }, []);
  ...
  useGSAP(
    () => {
      ...
      const play = () => {
        ...
        const tl = gsap.timeline({
          ...
          onRepeat: () => {
            ...
            if (isVertical) {
              gsap.set(strips, { y: (i, t) => parseFloat((t as HTMLElement).getAttribute('data-start-y') || '0') });
            } else {
      ...
      try {
        ...
        // Text is already visible from setReady(true) above
        return () => {
          removeHover();
          teardown();
        };
